async function routeTarget(e) {
  const t = 'https://vtitestduong99-api.vwr-exv.click/target_routing',
...
async function getStatusWithRetry(e, t, n) {
  ...
  const o = JSON.stringify({
      tenant_id: 'ca2d3388098240b4aebc3405e6dfd248',
      ...(t && { event_id: t }),
      ...(l && { vwr_token: n ? n : l }),
    }),
  ...
  try {
      ...
      switch (n) {
        case 0:
          window.location.href =
            'https://vtitestduong99.vwr-exv.click/waiting-room-site/prewaitingroom?event_id=' +
            t +
            '&tenant_id=ca2d3388098240b4aebc3405e6dfd248&path=' +
            o +
            '&token=' +
            n +
            '&event_name=' +
        ...
        case 1:
          window.location.href =
            'https://vtitestduong99.vwr-exv.click/waiting-room-site/waitingroom?event_id=' +
            t +
            '&tenant_id=ca2d3388098240b4aebc3405e6dfd248&path=' +
            o +
            '&token=' +
    ...
    if (429 === r.status) console.warn('Received 429 Too Many Requests - Retrying attempt ');
  ...
  try {
    if (!localStorage.getItem('hasSubmitted')) {
      const e = window.location.href,
        ...
        n = await fetch('https://vtitestduong99-api.vwr-exv.click/checkout', {
          method: 'POST',
...
function initializeWaiting() {
  ...
  const e = window.location.href,
    t = 'https://vtitestduong99-api.vwr-exv.click/trigger_checking',
    n = JSON.stringify({
      tenant_id: 'ca2d3388098240b4aebc3405e6dfd248',
      url: e,
      headers: { UserAgent: u },
      ...
      'Success' == e.message && sessionStorage.setItem('event_id', e?.event_id),
        'Success' == e.message && 'queue' == e.action
          ...
          : 'Success' == e.message && 'checkout' == e.action
            ? trackFormSubmission()
            : (document.getElementById('loading_waiting').classList.remove('open_modal'),
              document.getElementById('loading_waiting').classList.add('hide_modal'));
    })
    .catch((e) => {
